...
public:
    ...
    {  
        ...
        hasPhysics = false;
        baseModelType = BaseModelType::None;
    }
    ...
    // 템플릿을 사용하여 해당 컴포넌트가 존재하는지 확인하는 함수
    template<ComponentChild ComponentClass>
    bool HasComponent() const
    {
        // ComponentClass의 정적 클래스 이름을 가져와서 검색
        std::string componentName = ComponentClass::GetStaticClassName();
        
        // components 맵에서 해당 컴포넌트가 존재하는지 확인
    ...
    // BaseModel 타입 설정/조회
    void SetBaseModelType(BaseModelType type) { baseModelType = type; }
    BaseModelType GetBaseModelType() const { return baseModelType; }
...
protected:
    ...
    BaseModelType baseModelType; // BaseModel 타입
};