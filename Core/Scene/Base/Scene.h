...
public:
	...
	std::shared_ptr<class Object> AddObject(std::shared_ptr<class Object> object);	// 오브젝트를 추가하는 메소드
	std::shared_ptr<class Object> RemoveObject(const std::string& name);			// 해당 이름을 가진 오브젝트를 제거하는 메소드
	std::shared_ptr<class Object> RemoveObject(size_t index);						// 해당 인덱스에 있는 오브젝트를 제거하는 메소드
	...
	CameraContainer& GetCameraContainer() { return cameras; }						// 카메라 컨테이너를 반환하는 메소드
...
protected:
	...
	std::vector<std::shared_ptr<class Object>> objects;		// 씬에 포함된 오브젝트를 저장하는 배열
