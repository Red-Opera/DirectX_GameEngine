...
public:
    ...
    // PhysX 콜라이더용 정점 데이터 추출 메서드 추가
    bool GetPhysXVertices(std::vector<physx::PxVec3>& vertices,
...
private:
    ...
    // 원본 모델 데이터 저장 (PhysX 콜라이더 생성용)
    mutable std::vector<std::vector<physx::PxVec3>> meshVertices;
    mutable std::vector<std::vector<uint32_t>> meshIndices;
    mutable bool physxDataCached = false;

    float modelScale = 1.0f;  // 모델 스케일 저장
};