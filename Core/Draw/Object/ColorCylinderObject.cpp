...
ColorCylinderObject::ColorCylinderObject(Scale scale, GraphicResource::Image::Color color, bool isLit)
{
	...
	const auto geometryTag = "$ColorCylinder.X" + std::to_string(scale.x) + "Y" + std::to_string(scale.y) + "Z" + std::to_string(scale.z);
	...
	{
		Technique tech("Color Sphere", RenderingChannel::main);
...
void ColorCylinderObject::SetPosition(Vector3 position) noexcept
{
	this->position = position;
}

void ColorCylinderObject::SetRotation(Rotation rotation) noexcept
{
	this->rotation = rotation;
}

void ColorCylinderObject::SetScale(Scale scale) noexcept
{
	this->scale = scale;
}

DirectX::XMMATRIX ColorCylinderObject::GetTransformMatrix() const noexcept
{
...
void ColorCylinderObject::CreateControlWindow(const char* name) noexcept
{
	if (ImGui::Begin(name))
	{
		ImGui::Text("Position");
