...
ColorConeObject::ColorConeObject(float size, GraphicResource::Image::Color color, bool isLit) : size(size)
{
	...
	const auto geometryTag = "$ColorCone." + std::to_string(size);
...
void ColorConeObject::SetPosition(Vector3 position) noexcept
{
	this->position = position;
}

void ColorConeObject::SetRotation(Rotation rotation) noexcept
{
	this->rotation = rotation;
}

DirectX::XMMATRIX ColorConeObject::GetTransformMatrix() const noexcept
{
	return DirectX::XMMatrixRotationRollPitchYaw(rotation.x, rotation.y, rotation.z) *
		DirectX::XMMatrixTranslation(position.x, position.y, position.z);
}

void ColorConeObject::CreateControlWindow(const char* name) noexcept
{
	if (ImGui::Begin(name))
	{
		ImGui::Text("Position");
