...
namespace Graphic
{
	...
	void ColorBlend::SetTransparency(float transparencyInput) noexcept
	{
		assert(transparency);
	...
	float ColorBlend::GetTransparency() const noexcept
	{
		assert(transparency);
	...
	void ColorBlend::PipeLineSet(DxGraphic& graphic) noexcept
	{
		const float* transparencyDatas = transparency ? transparency->data() : nullptr;

		// 블렌딩 상태를 렌더링 파이프 라인에 입력
		GetDeviceContext(graphic)->OMSetBlendState(blendState.Get(), transparencyDatas, 0xffffffffu);
	}
}