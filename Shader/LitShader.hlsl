...
VertexOut VS(float3 localPosition : Position, float3 normal : Normal)
{
  ...
  //vertexOut.worldPosition = (float3) mul(float4(localPosition, 1.0f), transform);     // localPosition에서 worldPosition을 구함
...
float4 PS(float3 worldPosition : Position, float3 normal : Normal) : SV_Target
{
    // 정점에서 빛의 거리와 방향을 구함
