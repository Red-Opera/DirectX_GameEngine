...
HRESULT CreateDevice(UINT* createDeviceFlags, D3D_FEATURE_LEVEL* featureLevel, ID3D11Device** device, ID3D11DeviceContext** deviceContext)
{
    ...
    *createDeviceFlags |= D3D11_CREATE_DEVICE_DEBUG;
    ...
    HRESULT hr = D3D11CreateDevice(
        0,
        D3D_DRIVER_TYPE_HARDWARE,       // GPU 하드웨어 가속으로 실행함
        0,
        *createDeviceFlags,
...
{
    // 후면 버퍼 설정
    backBufferModeDESC->Width = WindowWidth;                                         // 후면 버퍼 너비
    backBufferModeDESC->Height = WindowHeight;                                       // 후면 버퍼 높이
    ...
    if (MSAAUSAGE)
    {
        ...
        massDESC->Quality = msaaQuality - 1; // CheckMultisampleQualityLevels로 통해서 얻어온 결과 값
    }
...
void CreateDepthStencilView(ID3D11Device* device, UINT msaaQuality, ID3D11Texture2D** depthStencilBuffer, ID3D11DepthStencilView** depthStencilView)
{
    ...
    HR(device->CreateTexture2D(&depthStencilDesc, 0, depthStencilBuffer));          // 2번째 매개변수 : 2D 텍스처에 기본적으로 입력될 값 (DepthStenil은 기본적으로 DirectX가 채워 줌)
    HR(device->CreateDepthStencilView(*depthStencilBuffer, 0, depthStencilView));   // 2번째 매개변수 : View DESC 값
}
